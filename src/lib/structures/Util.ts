...
export default class Util {
  ...
  public async loadCommands(): Promise<void> {
    return globby(`${this.directory}commands/**/*.js`).then(
      async (commands: string[]) => {
        for (const commandFile of commands) {
          const { name } = path.parse(commandFile);
          const { default: File } = await import(
            pathToFileURL(commandFile).toString()
          );
          ...
          if (!(command instanceof Command))
            throw new TypeError(
              `Command ${name} doesn't belong in commands directory.`
            );
  ...
  public async loadEvents(): Promise<void> {
    return globby(`${this.directory}events/**/*.js`).then(
      async (events: string[]) => {
        for (const eventFile of events) {
          const { name } = path.parse(eventFile);
          const { default: File } = await import(
            pathToFileURL(eventFile).toString()
          );
          if (!this.isClass(File))
          ...
          if (!(event instanceof Event))
            throw new TypeError(
              `Event ${name} doesn't belong in events directory.`
            );
          this.client.events.set(event.name, event);
          event.emitter[event.type](event.name, (...args: unknown[]) =>
            event.run(...args)
          );
        }
      }
