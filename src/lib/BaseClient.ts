...
> extends Client<Ready> {
  ...
  private validate(options: ClientOptions) {
    ...
    if (!options.owners.length)
      throw new Error("You must pass a list of owner(s) for the Client.");
    if (!Array.isArray(options.owners))
      throw new TypeError("Owner(s) should be a type of Array<String>.");
    ...
    this.defaultPermissions = new PermissionsBitField(
